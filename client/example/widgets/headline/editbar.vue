<template>
	<div class="edit-bar headline" v-if="activeElement.uuid==parentID">
		...
		<div class="third-bar" v-if="tab==3">
			<input type="color" id="color_picker" value="#fdffff" @mousedown.stop="" @change="fontColor($event)" />
			<button class="menu-btn" @mousedown.stop="fontSize()">{{parseInt(activeElement.fontSize)}}
				<i class="fa fa-caret-down"></i>
				<ul class="dropdown" v-if="sub_tab == 1">
					<li v-for="(x , i) in 18" @mousedown.stop="setFontSize(i,2*(x-1)+18)">{{2*(x-1)+18}}</li>
				</ul>
			</button>
			...
			<button class="menu-btn" @mousedown.stop="textAlign()"><i :class="'fas fa-align-'+activeElement.textAlign"></i><i class="fa fa-caret-down"></i>
			...
			<button class="menu-btn" @mousedown.stop="textLink()"><i class="fas fa-link"></i>
			</button>
			<button class="menu-btn" @mousedown.stop="updateText()" style="width:50px">DONE</button>
		</div>
...
export default{
	...
	methods : {
		...
		setFontFamily(val){
			this.$store.state.activeElement.fontFamily = val
			this.sub_tab = 0
		},
...
<style>
	...
	.headline .primary-bar{width:250px;}
	.headline .third-bar{width:505px;}
	.headline .dropdown{
	    position: absolute;
	    width: 100px;
	    color: black;
	...
	.headline .dropdown li i{float: none}
	.headline .fourth_bar{font-size: 15px; width:375px;}
	.input_edit{width:200px;}
</style>